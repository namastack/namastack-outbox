...
class TraceContextPropagator(
...
) : OutboxCreationInterceptor, OutboxDeliveryInterceptor {
    private val log = LoggerFactory.getLogger(TraceContextPropagator::class.java)
    ...
    private fun recreateSpan(attributes: Map<String, String>): Span.Builder? {
        try {
            val traceMap = attributes.toMap()
            var retriedTrace = false
            val spanBuilder =
                propagator.extract(traceMap) { carrier, key ->
