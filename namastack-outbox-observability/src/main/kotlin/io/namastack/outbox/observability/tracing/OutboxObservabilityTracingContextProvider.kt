 ...
 *
 * At **processing time**, the record-attempt observation wraps the record in an
 ...
 * trace. Handler observations then become children of that span.
 *
 ...
 * @since 1.7.0
 */
class OutboxObservabilityTracingContextProvider(
    private val tracer: Tracer,
    private val propagator: Propagator,
) : OutboxContextProvider {
    private val log = LoggerFactory.getLogger(OutboxObservabilityTracingContextProvider::class.java)
    ...
    override fun provide(): Map<String, String> {
        ...
        try {
            propagator.inject(context, carrier) { map, key, value ->
                log.trace("Injecting trace context: {}={}", key, value)
                map!![key] = value
            }
            return carrier
        } catch (ex: Exception) {
